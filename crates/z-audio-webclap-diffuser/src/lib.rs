...
use z_audio_dsp::{Diffuser, DiffuserParams, Effect, ParamId, ParamUnit, ProcessContext};
...
static PLUGIN_DEF: PluginDef = PluginDef {
    id: b"dev.zaudio.diffuser\0",
    name: b"Z Audio Diffuser\0",
    vendor: b"zukky\0",
    url: b"https://github.com/SuzukiDaishi/z-audio-dsp\0",
    version: b"0.1.0\0",
    description: b"Schroeder allpass diffuser built on z-audio-dsp\0",
    features: &[b"audio-effect\0", b"diffuser\0", b"stereo\0"],
    audio_inputs: 1,
    audio_outputs: 1,
    note_inputs: 0,
...
struct ZAudioWebDiffuser {
    diffuser: Diffuser,
    params: DiffuserParams,
    sample_rate: f32,
}

impl Plugin for ZAudioWebDiffuser {
    fn new() -> Self {
        let mut diffuser = Diffuser::default();
        diffuser.prepare(48_000.0, 128);
        Self {
            diffuser,
            params: DiffuserParams::default(),
    ...
    fn activate(&mut self, sample_rate: f64, max_frames: u32) {
        ...
        self.diffuser.set_params(self.params);
    }
    ...
    fn get_param(&self, id: u32) -> f64 {
        match id_to_param(id) {
            Some(ParamId::DiffuserMix) => self.params.mix as f64,
            Some(ParamId::DiffuserDiffusion) => self.params.diffusion as f64,
            Some(ParamId::DiffuserAllpassCount) => self.params.allpass_count as f64,
            Some(ParamId::DiffuserSize) => self.params.size as f64,
            Some(ParamId::DiffuserWidth) => self.params.width as f64,
            Some(ParamId::DiffuserOutputGain) => self.params.output_gain_db as f64,
            _ => 0.0,
        }
    }
    ...
    fn set_param(&mut self, id: u32, value: f64) {
        ...
        match param_id {
            ParamId::DiffuserMix => self.params.mix = value,
            ParamId::DiffuserDiffusion => self.params.diffusion = value,
            ParamId::DiffuserAllpassCount => self.params.allpass_count = value,
            ParamId::DiffuserSize => self.params.size = value,
            ParamId::DiffuserWidth => self.params.width = value,
            ParamId::DiffuserOutputGain => self.params.output_gain_db = value,
            _ => {}
        }
        self.diffuser.set_params(self.params);
    }
...
pub extern "C" fn _initialize() {
    init_plugin::<ZAudioWebDiffuser>(&PLUGIN_DEF);
}
