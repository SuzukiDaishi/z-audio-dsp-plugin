...
static PLUGIN_DEF: PluginDef = PluginDef {
    id: b"dev.zaudio.formula-piano\0",
    name: b"Z Audio Formula Piano\0",
    vendor: b"zukky\0",
    url: b"https://github.com/SuzukiDaishi/z-audio-dsp\0",
    version: b"0.1.0\0",
    description: b"Modal formula piano instrument built on z-audio-dsp\0",
    features: &[b"instrument\0", b"piano\0", b"stereo\0"],
    audio_inputs: 0,
    audio_outputs: 1,
    note_inputs: 1,
    ui_path: None,
...
impl Plugin for ZAudioWebPiano {
    fn new() -> Self {
        Self {
            piano: FormulaPiano::new(FormulaPianoConfig {
    ...
    fn activate(&mut self, sample_rate: f64, max_frames: u32) {
        ...
        self.piano = FormulaPiano::new(FormulaPianoConfig {
...
pub extern "C" fn _initialize() {
    init_plugin::<ZAudioWebPiano>(&PLUGIN_DEF);
}
