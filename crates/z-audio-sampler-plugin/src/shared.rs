...
//! assembly, zone-table staging — happens here, and the audio thread only
//! `try_lock`s prepared results at the top of each block.
//!
//! The webview editor forwards the exact WebCLAP `ZSMP` packets its UI
//! emits (see `z_audio_webclap_sampler::protocol`), so [`on_ui_message`]
...
//! directly.
//!
//! [`on_ui_message`]: SamplerShared::on_ui_message
//! [`queue_commit`]: SamplerShared::queue_commit
//! [`queue_clear`]: SamplerShared::queue_clear

use std::sync::{Mutex, MutexGuard};

use z_audio_webclap_sampler::engine::{SourceSample, ZoneSampler};
use z_audio_webclap_sampler::protocol::{self, UiMessage, ZoneDef};
...
pub enum SamplerUpdate {
    /// New zone table; `source` carries new PCM when a completed chunk
    /// upload preceded the commit (mirrors `ZoneSampler::commit_zones`).
    Commit {
...
pub struct SamplerShared {
    ...
    update: Mutex<Option<SamplerUpdate>>,
...
impl SamplerShared {
    ...
    pub fn mirroring(sampler: &ZoneSampler) -> Self {
        let (frames, sample_rate, channels) = sampler.source_info();
        Self {
            update: Mutex::new(None),
            notes: Mutex::new(Vec::with_capacity(MAX_PENDING_NOTES)),
            staging: Mutex::new(None),
            status: Mutex::new(SharedStatus {
                has_sample: sampler.has_sample(),
                channels,
                zone_count: sampler.zone_count() as u16,
                frames,
                sample_rate,
            }),
        }
    ...
    pub fn on_ui_message(&self, bytes: &[u8], reply: &mut dyn FnMut(&[u8])) {
        ...
        match msg {
            ...
            UiMessage::CommitZones(zones) => {
                let source = lock(&self.staging).take().map(|upload| SourceSample {
                    sample_rate: upload.sample_rate,
                    channels: upload.channels,
                    data: upload.data,
                });
                self.queue_commit(source, zones);
                self.push_status(reply);
            }
    ...
    /// Same bounds/finite-clamp rules as `ZoneSampler::upload_chunk`,
    ...
    /// Queues a zone-table commit (with new source PCM when `source` is
    /// `Some`) and updates the status mirror.
    pub fn queue_commit(&self, source: Option<SourceSample>, zones: Vec<ZoneDef>) {
        {
            ...
            status.zone_count = zones.len() as u16;
        }
        let mut slot = lock(&self.update);
        // A sourceless re-commit (marker drag) must not drop PCM that is
        // still waiting in an unapplied update — carry it forward.
        let carried = match slot.take() {
            Some(SamplerUpdate::Commit {
                source: Some(pending),
                ..
            }) if source.is_none() => Some(pending),
            _ => None,
        };
        *slot = Some(SamplerUpdate::Commit {
    ...
    pub fn queue_clear(&self) {
        ...
        *lock(&self.update) = Some(SamplerUpdate::Clear);
    }
    ...
    pub fn take_update(&self) -> Option<SamplerUpdate> {
...
mod tests {
    use super::*;
    use z_audio_webclap_sampler::engine::classic_zone;
    use z_audio_webclap_sampler::protocol::{
        encode_begin, encode_chunk, encode_clear, encode_commit, encode_note_preview,
        encode_status, OP_STATUS,
    };

    fn fresh() -> SamplerShared {
        SamplerShared::mirroring(&ZoneSampler::new(48_000.0))
    }
    ...
    fn ready_replies_with_the_mirrored_status() {
        ...
        sampler.set_source(source, vec![classic_zone(60, 1_000)]);
        let shared = SamplerShared::mirroring(&sampler);

        let mut replies: Vec<Vec<u8>> = Vec::new();
        shared.on_ui_message(READY_PACKET, &mut |bytes| replies.push(bytes.to_vec()));
        assert_eq!(
            replies,
            vec![encode_status(true, 1, 1, 1_000, 44_100.0).to_vec()]
        );
    }
    ...
    fn upload_and_commit_produce_a_source_update() {
        ...
        let mut replies: Vec<Vec<u8>> = Vec::new();
        shared.on_ui_message(&encode_commit(&[classic_zone(60, 1_000)]), &mut |bytes| {
            replies.push(bytes.to_vec())
        });

        let Some(SamplerUpdate::Commit {
            source: Some(source),
            zones,
        }) = shared.take_update()
        else {
    ...
    fn out_of_bounds_chunk_aborts_the_upload() {
        ...
        shared.on_ui_message(&encode_commit(&[classic_zone(60, 100)]), &mut no_reply());
        let Some(SamplerUpdate::Commit { source, .. }) = shared.take_update() else {
            panic!("expected a commit");
        };
        assert!(source.is_none(), "poisoned upload must not become a source");
        assert!(!shared.status().has_sample);
    }

    #[test]
    fn sourceless_recommit_carries_pending_pcm_forward() {
        // The UI re-commits zones (e.g. a marker drag) while a prior
        // sample+commit is still waiting for the audio thread. The PCM must
        // survive the replacement.
        let shared = fresh();
        shared.on_ui_message(&encode_begin(48_000.0, 1, 64), &mut no_reply());
        shared.on_ui_message(&encode_chunk(0, &[0.25; 64]), &mut no_reply());
        shared.on_ui_message(&encode_commit(&[classic_zone(60, 64)]), &mut no_reply());
        shared.on_ui_message(&encode_commit(&[classic_zone(64, 64)]), &mut no_reply());
        ...
        let Some(SamplerUpdate::Commit {
            ...
            panic!("expected the carried-forward source");
        };
        assert_eq!(source.frames(), 64);
        assert_eq!(zones[0].root, 64, "newest zone table wins");
    }
    ...
    fn clear_zeroes_the_status_and_queues_a_clear() {
        ...
        shared.queue_commit(
            ...
            vec![classic_zone(60, 128)],
        );
        let mut replies: Vec<Vec<u8>> = Vec::new();
        shared.on_ui_message(&encode_clear(), &mut |bytes| replies.push(bytes.to_vec()));
        assert!(matches!(shared.take_update(), Some(SamplerUpdate::Clear)));
        assert_eq!(replies.len(), 1);
        assert_eq!(replies[0][4], OP_STATUS);
        assert!(!shared.status().has_sample);
        assert_eq!(shared.status().zone_count, 0);
    }
}
