import {getHost, startHost, getWclap} from "./wclap-js/wclap.mjs?v=z-audio-host-5";
...
export default class ClapAudioNode {
	...
	async createNode(audioContext, pluginId, nodeOptions) {
		...
		if (!audioContext[this.#moduleAddedToAudioContext]) {
			let moduleUrl = new URL('./clap-audioworkletprocessor.mjs?v=z-audio-host-5', import.meta.url);
		...
		effectNode.getFile = async path => {
			...
			let match = Object.keys(files).find(key => key.endsWith('/' + suffix));
