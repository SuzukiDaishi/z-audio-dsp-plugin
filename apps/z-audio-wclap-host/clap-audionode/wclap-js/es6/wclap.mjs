	...
	constructor(config, hostImports) {
		...
		hostImports._wclapInstance = {
			...
			call32: (instancePtr, wasmFn, isPtrToFn, resultPtr, argsPtr, argsCount) => {
				...
				let targetFn = entry.functionTable.get(wasmFn);
				if (typeof targetFn !== 'function') {
					throw Error(`WCLAP function-table entry ${wasmFn} is ${targetFn}; table length ${entry.functionTable.length}; args ${args.join(',')}`);
				}
		...
		let makeHostFunctionsNative = hostFnEntries => {
			...
			for (let key in hostFnEntries) {
				...
				if (typeof hostFn !== 'function') {
					throw Error(`WCLAP host function-table entry ${entry.hostIndex} is ${hostFn}; table length ${this.#functionTable.length}; key ${key}`);
				}
